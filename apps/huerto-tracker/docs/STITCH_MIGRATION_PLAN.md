...
Rama de trabajo: `codex/stitch-ios-clone`.
...
- Coach: se retiró el spotlight automático heredado de Hoy, Calendario, Diario, Herramientas y Nueva planta; el acompañamiento queda integrado en tarjetas y consejos contextuales, sin overlays al entrar en las pantallas Stitch. La ruta `/coach-demo` conserva el frame explícito de demostración de Semillita para probar el flujo por separado.
...
- Limpieza de arquitectura visual: retirados `react-native-copilot`, `SemillitaTourProvider`, `SemillitaTooltip`, `useTourAutoStart`, `CoachMark`, `useCoachMark`, `CoachBubble` y el prototipo `/coach-demo`; el copy contextual de Nueva planta vive ahora bajo `plantNew.guidedDetails` y las claves del tour antiguo también se han retirado de los seis locales. Ajustes conserva el flujo real de cuenta, huerto y datos, e incorpora el selector de apariencia Claro/Oscuro/Sistema, accesibilidad (alto contraste y hápticos) y accesos diferenciados a permisos de cámara, fotos y notificaciones.
...
- QA actual: `npm run typecheck`, `npm run test -- --run` (16 archivos / 170 tests), `git diff --check` y `npx expo export` para web/Android/iOS pasan; los exports se verificaron en `apps/huerto-tracker/dist`. `expo-doctor` queda en 19/21 por duplicados de `node_modules` y versiones patch del SDK ya presentes en el workspace; no afecta al bundle generado ni a las rutas inspeccionadas.
- Preflight nativa: Metro vuelve a arrancar con Expo Go y el bundle nativo se genera para Android/iOS; las rutas de Herramientas, Recordatorios, Nueva planta y Notificaciones cargan sin errores. La instalación física conserva duplicados/versiones patch del SDK que `expo-doctor` reporta como mantenimiento pendiente del workspace.
...
- Cierre de esta migración: las rutas del catálogo de Stitch están presentes en Expo Router, el smoke web cubre pantallas principales, auxiliares y modales, y las pantallas de gestión de huertos y copias comparten la navegación inferior oficial. La revisión física en un iPhone sigue siendo una comprobación de dispositivo, no un bloqueo del bundle ni de la paridad de la interfaz.
