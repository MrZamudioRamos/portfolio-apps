...
function StitchSettingsScreen({ colors, router, appearance, garden, userEmail, isGuest, isPro, onAppearanceChange, onDeleteAccount, onDeleteData, onExport }: { colors: ReturnType<typeof useColors>; router: ReturnType<typeof useRouter>; appearance: 'Claro' | 'Oscuro' | 'Sistema'; garden: Garden | null; userEmail?: string; isGuest: boolean; isPro: boolean; onAppearanceChange: (value: 'Claro' | 'Oscuro' | 'Sistema') => void; onDeleteAccount: () => void; onDeleteData: () => void; onExport: () => void }) {
  const [contrast, setContrast] = useState(false);
  const [haptics, setHaptics] = useState(true);
  const rows = [
    { icon: 'leaf-outline' as const, title: 'Huerto activo', value: garden ? `${garden.name} (${garden.province})` : 'Sin huerto seleccionado', onPress: () => router.push('/gardens' as any) },
    { icon: 'thermometer-outline' as const, title: 'Ubicación meteorológica', value: garden?.province ?? 'Configura una provincia', onPress: () => Alert.alert('Ubicación meteorológica', garden ? `El tiempo se consulta para ${garden.province}.` : 'Añade una provincia en tu huerto para consultar el tiempo real.') },
  ...
  return <SafeAreaView style={[settingsStitch.container, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={settingsStitch.content}>
      <View style={[settingsStitch.header, { borderBottomColor: colors.border }]}><Pressable onPress={() => goBackOr(router)} accessibilityRole="button" accessibilityLabel="Volver a Mi Huerto" style={settingsStitch.back}><Ionicons name="chevron-back" size={22} color={colors.text} /><Text style={[settingsStitch.backText, { color: colors.text }]}>Mi Huerto</Text></Pressable><Text style={[settingsStitch.headerTitle, { color: colors.text }]}>Ajustes</Text><Pressable onPress={() => Alert.alert('Sobre Semilla', 'Semilla te acompaña a cuidar un huerto urbano sin adivinar cuándo regar. Versión 1.0.0.')} accessibilityRole="button" accessibilityLabel="Información"><Ionicons name="information-circle-outline" size={22} color={colors.primary} /></Pressable></View>
      <View style={[settingsStitch.profile, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[settingsStitch.avatar, { backgroundColor: colors.primary + '20' }]}><Ionicons name="person" size={26} color={colors.primary} /></View><View style={{ flex: 1 }}><Text style={[settingsStitch.profileName, { color: colors.text }]}>{isGuest ? 'Sesión local' : userEmail ?? 'Cuenta Semilla'}</Text><Text style={[settingsStitch.profileMeta, { color: colors.textSecondary }]}>{isPro ? 'Semilla Pro' : 'Plan gratuito'} · Tus datos permanecen en tu cuenta</Text></View><Pressable onPress={showProfileInfo} accessibilityRole="button" accessibilityLabel="Editar perfil"><Ionicons name="create-outline" size={19} color={colors.primary} /></Pressable></View>
      <Text style={[settingsStitch.active, { color: colors.textSecondary }]}>▣  Huerto activo: <Text style={{ color: colors.text, fontWeight: '800' }}>{garden?.name ?? 'Sin seleccionar'}</Text>{garden?.province ? ` · ${garden.province}` : ''}</Text>
