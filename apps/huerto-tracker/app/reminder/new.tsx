...
import { useReminders, FREQUENCY_LABELS, type ReminderFrequency } from '@portfolio/notifications';
...
export default function ReminderNewScreen() {
  ...
  function handleTypeChange(t: ReminderType) {
    setType(t);
    setTitle(REMINDER_TYPE_CONFIG[t].defaultTitle);
  }
        ...
        <View style={s.body}>
          ...
          <View style={[s.preview, { backgroundColor: colors.surfaceAlt, borderColor: colors.border }]}>
            ...
            <Text style={[{ color: colors.textDisabled, fontSize: fontSize.xs, marginTop: 4 }]}>
              {FREQUENCY_LABELS[frequency]} · {String(hour).padStart(2, '0')}:{String(minute).padStart(2, '0')}
            </Text>
          </View>
        </View>
      </ScrollView>
