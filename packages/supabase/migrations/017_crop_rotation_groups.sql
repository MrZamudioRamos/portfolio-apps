-- Curated botanical groups for history-based rotation context.
-- Based on RHS crop rotation guidance and University of Minnesota Extension.
-- Ambiguous, custom, ornamental, and perennial crops remain NULL: the app
-- does not manufacture a rotation family when its reference data is unknown.
alter table crops add column if not exists rotation_group text;

update crops set rotation_group = case
